...
public class CafRepository(AppDbContext db) : ICafRepository
{
    public async Task<IReadOnlyList<ExpedienteCaf>> GetAllAsync(CancellationToken ct = default)
        => await db.ExpedientesCaf.AsNoTracking()
            .OrderByDescending(e => e.Anio)
            .ThenBy(e => e.Expediente)
            .ThenBy(e => e.Op)
            .ToListAsync(ct);

    public Task<ExpedienteCaf?> GetByIdAsync(int id, CancellationToken ct = default)
        => db.ExpedientesCaf.FirstOrDefaultAsync(e => e.Id == id, ct);
